...
def load_reference_front(ref_front_path):
    ...
    if not ref_front_path.exists():
        ...
        print(f"[WARN] IGD+ will return inf - this is acceptable for first runs")
        return None
...
def main():
    parser = argparse.ArgumentParser(description='Production Run V8')
    ...
    log_file = f"production-run-v8-{run_id}-{timestamp}.log"
    ...
    log("=" * 80)
    log("PRODUCTION RUN V8 - MODULAR ARCHITECTURE")
    log("=" * 80)
    log(f"Run ID: {run_id}")
    log(f"Seed: {seed}")
    ...
    log("V8 Configuration:")
    log(f"  Community: {config['community_name']} (pop {config['population']})")
    log(f"  Wind CF: {config['wind_cf_path'].name} (CASES High Point 30%)")
    log(f"  Battery bounds: [0, {bounds['battery_kwh'][1]}] MWh (CORRECTED!)")
    log(f"  PV area: {config['area_available_pv_m2']/1000:.0f}k m2")
    log(f"  Wind area: {config['area_available_wind_m2']/1000:.0f}k m2")
    log(f"  RE cap: {config['renewable_fraction_max']:.0%}")
    log(f"  Additional metrics: RE%, Excess%, LCOE, Fuel")
    log("")
    ...
    callback = NSGA3ProgressCallback(
        ref_point=ref_point,
    ...
    log("Callback configured:")
    log(f"  Log every: 5 generations")
    log(f"  Early stop: 20 gen stagnation (tol=0.01)")
    log("")
    ...
    try:
        ...
        log("=" * 80)
        early_stopped = True
        n_gen_actual = callback.metrics_history[-1]['generation']
        ...
        for i in range(len(F_final)):
            ...
            _, constraints, _ = simulate_system(decision_vars, config)
    ...
    log("Calculating metrics for each solution (includes simulation)...")
