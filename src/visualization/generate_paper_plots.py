...
\end{table}
        Tables 4-6 (CMOPEO 2025): Representative solutions from Pareto front

        Selection criteria:
        - Min NPC (lowest cost)
        - Min LPSP (highest reliability)
        - Min CO2 (lowest emissions)
        - Min Gini (highest equity)
...
\end{table}
    Generate all expected plots for paper.

    Args:
        project_root: Path to project root
        metrics_files: List of convergence metrics CSV files (if None, auto-detect latest)
        pareto_files: List of Pareto front CSV files (if None, auto-detect latest)
        save_format: 'pdf', 'png', or 'svg'
    Usage:
