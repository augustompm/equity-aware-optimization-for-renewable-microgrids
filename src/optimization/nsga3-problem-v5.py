...
class MicrogridOptimizationProblemV5(Problem):

    def __init__(self, system_config, bounds_config):
        ...
        xl = np.array([
            bounds_config['pv_kw'][0],
            bounds_config['wind_kw'][0],
            bounds_config['battery_kwh'][0],
            bounds_config['diesel_kw'][0]
        ])

        xu = np.array([
            bounds_config['pv_kw'][1],
            bounds_config['wind_kw'][1],
            bounds_config['battery_kwh'][1],
            bounds_config['diesel_kw'][1]
        ])
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i in range(n_pop):
            decision_vars = {
                'n_pv_kw': X[i, 0],
                'n_wind_mw': X[i, 1] / 1000.0,
                'e_battery_mwh': X[i, 2] / 1000.0,
                'p_diesel_mw': X[i, 3] / 1000.0
            }
