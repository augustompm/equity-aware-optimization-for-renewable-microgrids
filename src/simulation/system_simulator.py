...
def simulate_system(decision_vars, system_config,
    ...
    if use_v3 and V3_AVAILABLE:
        ...
        aggregate_load_mwh = results_df['load_mw'].values
        renewable_production_mwh = (results_df['pv_mw'].values +
                                    results_df['wind_mw'].values)
        hourly_costs = results_df['diesel_cost_usd'].values
        gini_value = objective_gini_adapted(
            hourly_costs,
            aggregate_load_mwh,
        ...
        cost_per_hour = results_df['diesel_cost_usd'].values
        gini_value = objective_gini(cost_per_hour)
